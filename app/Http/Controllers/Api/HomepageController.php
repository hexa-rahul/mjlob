...
class HomepageController extends Controller
{
     ...
     public function getMasterCategory(Request $request)
    {
        ...
        $response    = MasterCategory::where('parent_id', 0)->where('deleted_at', null)->latest()->paginate();
        
        foreach ($response as $key => $value) {

            if($value->is_sub_category_exist==1){
                
                $getData                     = $this->getRelativesubdata($value->id);
                $response[$key]->sub_data    = $getData; 
               
            }else{
                $response[$key]->sub_data    = [];
            }
        }
    ...
    public function getRelativedata($parent_id=''){

        return $response    = MasterCategory::where('parent_id', $parent_id)->where('deleted_at', null)->with('subData')->latest()->get();
    }

    public function getRelativesubdata($parent_id=''){

        $response    = MasterCategory::where('parent_id', $parent_id)->where('deleted_at', null)->latest()->get();
